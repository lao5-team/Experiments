...
;(function( $, window, document, undefined ) {
    ...
    var $window = $(window),
        ...
        defaults = {
            ...
            loadingMsg: '<div style="text-align:center;padding:10px 0; color:#999;">数据加载中...</div>', // loading html
    ...
    Waterfall.prototype = {
        ...
        _requestData: function(callback) {
            ...
            var getFollowInfo = function (authorIdList) {
                $.ajax({
                    type: 'POST',
                    url: tumAppInst.options.hostName + 'follow/artist/isfollow',
                    data: JSON.stringify(authorIdList),
                    headers: headers,
                    dataType: dataType,
                    contentType: 'application/json',
                    beforeSend: function(xhr) {
                        //TODO: Workaround Mark
                        var token = localStorage.getItem('ngStorage-token').replace(/"/g,"");
                        xhr.setRequestHeader('Authorization', 'Bearer ' + token);
                    },
                    success: function(data) {
                        $.each(authorList.results, function (index, value) {
                            value.isFollow = tumAppInst.findEleInArray(data, 'id', value.uid).isFollow;
                        });

                        self._handleResponse(authorList, callback);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var uid = localStorage.getItem('ngStorage-uid').replace(/"/g,"");
                    uid = uid ? uid : null;
                    ...
                    $.each(data.results, function (index, value) {
                        ...
                        value.first = value.cover[0];
                        value.second = value.cover[1];
                        value.third = value.cover[2];
                        value.fourth = value.cover[3];
                        authorIdList.push(value.uid);
                    });

                   /* for(var i=0;i<data.results.length;i++){
                        data.results[i].first = data.results[i].cover[0];
                        data.results[i].second = data.results[i].cover[1];
                        data.results[i].third = data.results[i].cover[2];
                        data.results[i].fourth = data.results[i].cover[3];
                        authorIdList.push();
                    }*/

                    getFollowInfo(authorIdList);
                },
                error: function(jqXHR) {
                    self._responeseError('error');
                }
